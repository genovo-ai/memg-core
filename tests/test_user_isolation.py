...
class TestUserIsolation:
    ...
    def test_search_data_isolation(self):
        ...
        user1_statements = [m.payload["statement"] for m in user1_results.memories] + [
            n.payload["statement"] for n in user1_results.neighbors
        ]
        user2_statements = [m.payload["statement"] for m in user2_results.memories] + [
            n.payload["statement"] for n in user2_results.neighbors
        ]
    ...
    def test_deletion_access_control(self):
        ...
        total_user1_results = len(user1_results.memories) + len(user1_results.neighbors)
        assert total_user1_results > 0, "Memory should still exist for original user"
    ...
    def test_hrid_scoping_per_user(self):
        ...
        total_user1_results = len(user1_results.memories) + len(user1_results.neighbors)
        total_user2_results = len(user2_results.memories) + len(user2_results.neighbors)
        assert total_user1_results == 1, "User1 should see exactly 1 result"
        assert total_user2_results == 1, "User2 should see exactly 1 result"

        # Get first memory from either memories or neighbors
        user1_all = user1_results.memories + user1_results.neighbors
        user2_all = user2_results.memories + user2_results.neighbors
        user1_content = user1_all[0].payload["statement"]
        user2_content = user2_all[0].payload["statement"]
    ...
    def test_memory_type_filtering_with_isolation(self):
        ...
        user1_note_hrids = [m.hrid for m in user1_notes.memories] + [
            n.hrid for n in user1_notes.neighbors
        ]
    ...
    def test_comprehensive_multi_user_isolation(self):
        ...
        for user in users:
            ...
            assert total_results == 2, f"User {user} should see exactly 2 results"

            # Verify all results belong to this user - check both memories and neighbors
            all_memories = results.memories + results.neighbors
            for memory in all_memories:
                statement = memory.payload["statement"]
    ...
    def test_edge_cases_user_isolation(self):
        ...
        for i, user in enumerate(similar_users):
            ...
            total_results = len(results.memories) + len(results.neighbors)
            assert total_results == 1, f"User {user} should see exactly 1 result"

            all_memories = results.memories + results.neighbors
            statement = all_memories[0].payload["statement"]
        ...
        total_special_results = len(special_results.memories) + len(special_results.neighbors)
        assert total_special_results == 1, "Special character user_id should work"

        # Other users should not see special user's data
        normal_results = search("special character", "normal_user", limit=10)
        total_normal_results = len(normal_results.memories) + len(normal_results.neighbors)
        assert total_normal_results == 0, "Normal user should not see special user's data"
