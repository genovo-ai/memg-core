...
class TestPublicAPIInterface:
    ...
    def test_search_returns_hrid_only(
        ...
        assert len(results.memories) > 0, "Should find the added memory"

        # Check that results contain no UUIDs - check both seeds and neighbors
        for memory_seed in results.memories:
            test_helpers.assert_no_uuid_exposure(memory_seed)

            # Should have HRID in result
            assert hasattr(memory_seed, "hrid"), "MemorySeed should have HRID"
            test_helpers.assert_hrid_format(memory_seed.hrid, "note")

        for memory_neighbor in results.neighbors:
            test_helpers.assert_no_uuid_exposure(memory_neighbor)

            # Should have HRID in result
            assert hasattr(memory_neighbor, "hrid"), "MemoryNeighbor should have HRID"
            test_helpers.assert_hrid_format(memory_neighbor.hrid, "note")
    ...
    def test_delete_memory_accepts_hrid(self, predictable_user_id: str, sample_note_data: dict):
        ...
        found_hrids = [m.hrid for m in results.memories] + [n.hrid for n in results.neighbors]
    ...
    def test_user_isolation_in_api(self, sample_note_data: dict):
        ...
        user1_hrids = [m.hrid for m in user1_results.memories] + [
            n.hrid for n in user1_results.neighbors
        ]
...
class TestPublicAPISearchFiltering:
    ...
    def test_search_limit_parameter(self, predictable_user_id: str):
        ...
        assert total_results <= 3, f"Should return at most 3 results, got {total_results}"
        assert total_results > 0, "Should return some results"
    ...
    def test_empty_search_query(self, predictable_user_id: str, sample_note_data: dict):
        ...
        assert isinstance(results, SearchResult), (
            "Should return a SearchResult even with empty query"
        )
...
class TestNewAPIFunctionality:
    ...
    def test_update_memory_basic(
        ...
        total_results = len(results.memories) + len(results.neighbors)
        assert total_results > 0, "Should find updated memory"
