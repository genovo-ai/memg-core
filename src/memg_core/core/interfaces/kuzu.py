...
class KuzuInterface:
    ...
    def neighbors(
        ...
        try:
            ...
            # Only validate UUID format if id_type is UUID
            if id_type.upper() == "UUID" and not self._is_uuid_like(node_id):
                raise ValueError(f"node_id must be a UUID when id_type=UUID, got: {node_id}")
